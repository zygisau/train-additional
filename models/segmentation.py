...
def get_segmentation_model(backbone, feature_indices, feature_channels, arch):
    ...
    model = SegmentationEncoder(
        backbone, feature_indices, feature_channels, arch, diff=True)
    unet = UNet(model, feature_channels, 1, bilinear=True,
                concat_mult=1, dropout_rate=0.3, arch=arch)
    # unet = UNetSmall(model, feature_channels, 1, bilinear=True, concat_mult=1)
    return unet


# for segmentation task
def segmentation_model(backbone, feature_indices, feature_channels, arch):
    """Creates a UNet from a pretrained backbone
    ...
    Returns:
        [type]: [description]
    """
    model = SegmentationEncoder1input(
        backbone, feature_indices, feature_channels, arch, diff=True)
    unet = UNet(model, feature_channels, 1, bilinear=True,
                concat_mult=1, dropout_rate=0.3, arch=arch)
    # unet = UNetSmall(model, feature_channels, 1, bilinear=True, concat_mult=1)
...
class SegmentationEncoder1input(torch.nn.Module):
    def __init__(self, backbone, feature_indices, feature_channels, arch, diff=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        """
        feats = [x]
        if 'swin' in self.arch:
            for i, module in enumerate(self.encoder.features.children()):
                # print(i,module)
                x = module(x)
                # x2 = module(x2)
                ...
                if i in self.feature_indices:
                    feats.append(x)
                    # print(self.concatenate(x1, x2).shape)
...
class UNet(nn.Module):
    def __init__(self, encoder, feature_channels, n_classes, concat_mult=2, bilinear=True, dropout_rate=0.5, arch='wideresnet'):
    ...
    def forward(self, *in_x):
        ...
        features = features[1:]
        x = features[-1]
        # len(features)-2, decrease to 0, corresponding to the settings above
        for i in range(len(features) - 2, -1, -1):
            ...
            if features[i].shape[-1] != x.shape[-1]:
                x2 = F.interpolate(features[i], scale_factor=4)
            else:
                x2 = features[i]
...
class UNetSmall(nn.Module):
    def __init__(self, encoder, feature_channels, n_classes, concat_mult=2, bilinear=True):
        ...
        self.aggregate = nn.Conv2d(len(
            feature_channels) * feature_channels[0], feature_channels[0], kernel_size=3, stride=1, padding=1, bias=False)
