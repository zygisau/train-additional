...
from temp.siamunet_diff_old import SiamUnet_diff
...
class SiamLightning(L.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        image1, image2, y = batch
        y_pred = self.model(image1, image2).squeeze()
    ...
    def test_epoch_end(self, outputs):
        ...
        for results_dict in outputs:
            y_true = np.append(
                y_true, results_dict["y_true"].cpu().detach().numpy())
            y_pred = np.append(
                y_pred, results_dict["y_pred"].cpu().detach().numpy())
