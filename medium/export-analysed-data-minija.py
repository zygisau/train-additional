...
def proceed(bounds: ee.Geometry.Rectangle, date_range: [str, str], bands: List[str], pixel: [float, float], moving_steps: int):
    ...
    projection = coll.first().select(1).projection()
...
if __name__ == '__main__':
    ...
    date_range = ['2023-09-01', '2023-12-30']
    ...
    filtered_coll, projection = filter_data_by_median(
        bounds, date_range, bands, points, moving_steps)

    print("Collection successfully filtered")

    dois = [ee.Date('2023-08-14'), ee.Date('2023-12-03')]
    samples = [
        filtered_coll.filterDate(dois[0], dois[0].advance(
            1, 'month')).sort('CLOUDY_PIXEL_PERCENTAGE'),
    ...
    for img, name in sampled_images:
        ...
        img = img.clip(rect).reproject(projection)
        export_lake(img, rect, f'{date}', 'analysed_final/minija_priekule',
                    scale=10, crs='EPSG:3857')
        export_lake(img, rect, f'{date}_rgb', 'analysed_final/minija_priekule', [
