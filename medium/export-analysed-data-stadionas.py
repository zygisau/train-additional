...
if __name__ == '__main__':
    ...
    projection = filtered_coll.first().select(0).projection()

    for img, name in sampled_images:
        print(f'Exporting {name} image')
        # get date
        date = img.date().format('YYYY-MM-dd').getInfo()
        img: ee.Image = img.clip(rect)
        export_lake(img, rect, f'{date}', 'analysed_final/stadionas',
                    scale=10, crs='EPSG:3857')
        export_lake(img, rect, f'{date}_rgb', 'analysed_final/stadionas', [
