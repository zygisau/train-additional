...
if __name__ == '__main__':
    ...
    date_range = ['2019-01-01', '2024-01-03']
    ...
    filtered_coll: ee.ImageCollection = ee.ImageCollection('COPERNICUS/S2')\
        .filterDate(date_range[0], date_range[1])\
        .filterBounds(bounds)
    # filter_data_by_median(
    ...
    samples = [
        filtered_coll.filterDate(ee.Date('2023-01-11'), ee.Date('2023-01-13')),
        filtered_coll.filterDate(ee.Date('2023-08-11'), ee.Date('2023-08-13')),
    ]
    ...
    for img, name in sampled_images:
        ...
        img = img.clip(rect)
        export_lake(img, rect, f'sysa_{date}', 'analysed_final/minija-sysa',
                    scale=10, crs='EPSG:3857')
        export_lake(img, rect, f'sysa_{date}_rgb', 'analysed_final/minija-sysa', [
